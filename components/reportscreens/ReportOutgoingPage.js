...
import { getAllOut } from '../database'; // Ensure correct path to database file

const ReportOutgoingPage = ({ navigation }) => {
  const [tableData, setTableData] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getAllOut();
...
export default ReportOutgoingPage;
